...
class TestFleetWorkOrder(TransactionCase):
    def setUp(self, *args, **kwargs):
        ...
        self.partner1 = self.env.ref("base.res_partner_19")
        ...
        self.rule = self.env["procurement.rule"].search(criteria, limit=1)
    ...
    def _process_procurement(self, picking):
        ...
        criteria = [("move_dest_id", "=", move1.id)]
        procurement = self.obj_procurement.search(criteria, limit=1)[0]
        procurement.write({"rule_id": self.rule.id})
        procurement.run()
        procurement.check()
